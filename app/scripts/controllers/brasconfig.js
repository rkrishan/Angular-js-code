...
angular.module('specta')
  .controller('BrasConfigCtrl', function ($scope, $state, $stateParams, globalConfig, SweetAlert, dbService, httpService, currentUser){
    ...
    $scope.isUnique = function(name){
        ...
        name = name.trim();
        $scope.errMsg = null;
        var tmpObj = dbService.unique($scope.list, 'Bras', name);
        console.log(tmpObj);
        if(tmpObj.length > 0) $scope.errMsg = 'Bras already exits.';
    }
    ...
    $scope.save = function(tmpdata){
        ...
        if($scope.errMsg == null){
            ...
            if( $stateParams.id && $stateParams.id != '' ){
                ...
                var url = dbService.makeUrl({collection: 'lku_bras', op: 'upsert', id: $stateParams.id});
                ...
                // return;
                var url = dbService.makeUrl({collection: 'lku_bras', op:'create'});
    ...
    $scope.cancle = function(){
        $state.go('index.systemconfig', {tab: 'bras'});
    }
});