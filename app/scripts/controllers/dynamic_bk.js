...
angular.module('specta')
    .controller('DynamicCtrl', function ($scope, $rootScope, $location, $http, $state, $interval,filterService, $timeout,$sce, $stateParams, $filter, $uibModal, NgMap, globalData, globalConfig, ChartService, SweetAlert, UserProfile, socket, NgTableParams, httpService, dbService, highchartProcessData){
    ...
    if(angular.isDefined($stateParams.id)){
        ...
        httpService.get(url).then(function(response){
            ...
            if( $scope.report.filter && $scope.report.filter.indexOf('segment') > -1 )
                $scope.loadSegment();
    ...
    function checkPageIsFavorite(page, pageId){
        ...
        var query = JSON.stringify({ page: page, pageId: pageId });
        var params = 'query=' + encodeURIComponent(query);
    ...
    $scope.addFavorite = function(favorite){
        if(favorite == true){
            var tmp = {pageId : $stateParams.id, userId : $scope.userProfile.userId, page: pageName};
    ...
    /*IBox*/
        $scope.redirectToOtherPage = function(params, component){
            redirectToOtherPage(params, component);
        }
        ...
        function processGauge(component, data){
            ...
            if( component.unit == 'speed' ){
                ...
                if( component.unit2 == 'KB' ){
                    gaugedata = Number((data[component.kpi] / Math.pow(2, 10)).toFixed(parseInt(component.dataDecimal)) );
                    console.log('gauge', component.title, 'unit2 = KB', gaugedata);
                    gaugeunit = 'KB';
                }
                else if( component.unit2 == 'MB' ){
                    gaugedata = Number((data[component.kpi] / Math.pow(2, 20)).toFixed(parseInt(component.dataDecimal)) );
                    console.log('gauge', component.title, 'unit2 = MB', gaugedata);
                    gaugeunit = 'MB';
                }
                    ...
                    console.log('gauge', component.title, 'unit2 = gb', gaugedata);
                    gaugeunit = 'GB';
                }
            }
            else{
            ...
            $timeout(function() {
                ...
                /*if(gaugedata < component.threshold)
                    var color = hexToRgbA(component.color);
                else if(gaugedata >= component.threshold)
                    var color = hexToRgbA(component.thresholdcolor);

                console.log(gaugedata, component.threshold, gaugedata < component.threshold, color);*/
                
            }, 10);
            console.log('gauge --->>>>', gauge.component.title, gauge.data);
