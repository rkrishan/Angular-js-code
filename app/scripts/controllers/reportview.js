...
angular.module('specta')
    .controller('ReportViewCtrl', function ($scope, $location, $http, $state, $filter, NgMap, globalConfig, $interval, $stateParams, ChartService, SweetAlert, UserProfile,$rootScope,socket,$timeout,NgTableParams) {
    ...
    /*ChartJs*/
        ...
        function processChartSingleSeriesReplaceData(component, data1){
            ...
            _.forEach(data1, function(data, key){
                ...
                var keyindex = $.inArray( label, chartData.labeldata );
                // console.log(keyindex, label);
                // console.log('processChartSingleSeriesReplaceData Data',data[component.series]);
                if(keyindex > -1){
                    console.log('processChartSingleSeriesReplaceData Data > -1',data[component.series]);
                    chartData.tempData[keyindex] = data[component.series];
                }
                else{
                    //console.log('processChartSingleSeriesReplaceData Data <= -1',data[component.series]);
                    if( data[component.series] ){
                        chartData.labeldata.push( label );
                        chartData.data.push( data[component.series] );
                        //console.log('processChartSingleSeriesReplaceData Pushing Data',data[component.series]);
                        chartData.tempData.push( data[component.series] );
                    }
                }
            });
        ...
        function processChartJSSingleSeriesReplaceData(component, data1){
            var chartData = $scope.displaydata[component._id];
            if( (angular.isDefined(chartData.statement) && chartData.statement.eventPublish == 'Combined') || component.data == 'DBPull' ){
                chartData.labeldata = [];
                chartData.data = [];
            ...
            _.forEach(data1, function(data, key){
                ...
                if( data[component.series] ){
                    chartData.labeldata.push( label );
                    chartData.data.push( data[component.series] );
                    console.log('processChartJSSingleSeriesReplaceData Pushing Data',data[component.series]);
                    chartData.tempData.push( data[component.series] );
                }
            });
