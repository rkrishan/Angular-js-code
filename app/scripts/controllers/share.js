...
angular.module('specta')
    .controller('ShareCtrl', function ($scope, $rootScope, $location, $state, $interval, filterService, $timeout,$sce, $stateParams, $filter, $uibModal, NgMap, globalData, globalConfig, ChartService, SweetAlert, UserProfile, socket, NgTableParams, httpService, dbService, highchartProcessData, currentUser, utility, dataFormatter){
    ...
    if(angular.isDefined($stateParams.id)){
        ...
        httpService.get(url).success(function(response){
            if(response.length == 0){
                swal('', 'Module not found ', 'warning');
                return;
            }
            response = response[0]
            $scope.headerName = response.title;
            $state.current.data.currentPage = response.title;
            
            $scope.loadPages(response)
    ...
    var z = 0
    function getModule(components, cb){
        console.log(z, components.length, components[z])
        if(components[z]){
            var item = angular.copy(components[z].component)
            if( Object.keys(item).length < 5){
                var query  = '{_id: ObjectId("'+item._id+'")}'
                var params = 'query=' + encodeURIComponent(query)
                var url    = dbService.makeUrl({collection: 'modules', op:'select', params: params})
                httpService.get(url).success(function(res){
                    if(res.length == 1){
                        components[z].component = res[0]
                        components[z].component.width = item.width
                        z++
                    }
                    else{
                        components.splice(z, 1)
                    }
                    getModule(components, cb)
                })
            }
            else{
                z++
    ...
    var origionalModule = {}
    $scope.loadPages = function(module){
        console.log(module)
        module.width = $stateParams.width
        var obj = {
            components : [{
                component : module,
        ...
        $scope.setCurrentPage(obj);
        return;
        var query = JSON.stringify({ 'dashboardId': '59e467ab7a93d8086f7805da' });
        var params = 'query=' + encodeURIComponent(query);
        ...
        httpService.get(url).then(function(response){
            ...
            if(response.data.length > 0){
                origionalModule = angular.copy(response.data[0])
	            getModule(response.data[0].components, function(res){
                    console.log('getModule', res)
                    response.data[0].components = res
    ...
    $scope.removeComponent = function (component1){
        ...
        function (isConfirm) {
            if(isConfirm){
                ...
                _.forEach($scope.currentPage.components, function(item, k){
                    _.forEach(item, function(value, key){
                        if(value.component._id != component.component._id){
                            ...
                            tempArr.push(value)
                            var obj = _.filter(origionalModule.components, function(tt){
                                return tt.component._id == value.component._id
                            })[0]
                            // var obj = { component: {_id : value.component._id, width: value.component.width}, componentType: value.componentType }
                            updateArr.push(obj)
                        }
                    })
                })
                ...
                var update = {
					dashboardId: $scope.currentPage.dashboardId,
					_id        : $scope.currentPage._id,
					components : updateArr
                }
                backgroundUpdatePage(update)
            }
        })
    }
    ...
    /*Highchart*/
        ...
        //stacked Bar
            ...
            function highchartStackedBarReplace(component, data1){
                ...
                // console.log(component, data1.length, data1[0])
