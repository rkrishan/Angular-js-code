...
angular.module('specta')
    .controller('DynamicCtrl', function ($scope, $rootScope, $location, $state, $interval, filterService, $timeout,$sce, $stateParams, $filter, $uibModal, NgMap, globalData, globalConfig, ChartService, SweetAlert, UserProfile, socket, NgTableParams, httpService, dbService, highchartProcessData, currentUser){
    ...
    $scope.backPage = function(){
        var url = $stateParams.params.returnPath.split('/');
        console.log(url);
        if(url[2] == 'dashboards')
        $state.go('index.dashboards', {id: url[3], params: null, filterParams: null, name: null});
    }
    ...
    if(angular.isDefined($stateParams.id)){
        ...
        httpService.get(url).then(function(response){
            ...
            var day = 1;
            var from = $filter('date')( new Date().getTime(), "yyyy-MM");
            ...
            if($scope.report.filter){
                ...
                if( $scope.report.filter.location ) $scope.loadLocation();

                if( $scope.report.filter.device ) $scope.loadDevice();
                
                if( $scope.report.filter.segment ) $scope.loadSegment();

                if( $scope.report.filter.plan ) $scope.loadPlan();

                if( $scope.report.filter.rat ) $scope.loadRat();
        ...
        if(type == 'iBox_Multi_no_Header'){
            ...
            isStatement(component, function(res){
                if(res){
                    ...
                    else if(type == 'col_extended_table')       subscribeComplexTable(component);
                    else if(type == 'map')                      subscribeMap(component);
                    else if(type == 'gauge')                    subscribeGauge(component);
    ...
    function subscribeChart(component){
        console.log('component', component.options, component);
        var fields = JSON.stringify(["options"]);
    ...
    /*IBoxMulti*/
        ...
        function processMultiIboxData(component, box, data){
            ...
            if(box.unit == 'percent'){
                ...
                // console.log(newValue, box.index);
                iboxData.data = (box.dataDecimal) ? newValue.value.toFixed(box.dataDecimal) : newValue.value;
            ...
            if(component.mapType == 'simple'){
                ...
                _.forEach(data1, function(data, key){
                    if(data.Country || data.countrycode){
                        var tmp = dbService.unique(globalData.countryLookUpObj, 'Country', data.Country);
                        if(tmp.length == 0){
                            tmp = dbService.unique(globalData.countryLookUpObj, 'countrycode', data.countrycode);
                            if(tmp.length == 0) return;
                        }
                        data.latitude = tmp[0].Latitude;
                        data.longitude = tmp[0].Longitude;
                    }
                    ...
                    for(var i in component.infoboxDataField){
                        ...
                        if(component.infoboxDataField[i] == 'Traffic' || component.infoboxDataField[i] == component.dataValue){
    ...
    /*Highchart*/
            ...
            function changeHighChartData(component, converted){
                ...
                _.forEach(component.series, function(value, seriesCount){
                    for(var i = 0; i < chartData.tempData[seriesCount].length; i++) {
                        ...
                        chartData.options.series[seriesCount].data[i] = Number(parseFloat(val) );
                    }
                });
            }
        ...
        //Pie Chart
                    ...
                    if( component.chartType == "Pie" ){
                        component.chartOptions.series[0].point = {
                            events:{
                                click: function (event){
                                    var label = this.name;
                                    var params = {Key: component.labels, unit: component.chartUnit, value: this.y, label: label};
                                    redirectToOtherPage(params, component);
                                }
                            }
                        };
                ...
                if(typeof(component.series) == 'object' && data1.length > 0){
                    ...
                    for(var item in component.series){
                        if(angular.isDefined(component.pieColor) && angular.isDefined(component.pieColor[component.serieslist.list[item]]) )
                            chartData.options.series[0].datalist.list[item]= {name: component.serieslist.list[item], y: data1[data1.length-1][component.serieslist.list[item]], color: component.pieColor[component.serieslist.list[item]]};
                        else
                            chartData.options.series[0].datalist.list[item]= {name: component.serieslist.list[item], y: data1[data1.length-1][component.serieslist.list[item]]};
                    }
                    chartData.updateTime = new Date();
                }
                else{
            ...
            function changeSingleSeriesHighchartData(component, converted){
                ...
                for(var i=0; i<chartData.tempData.length; i++){
                    ...
                    val = getConvertedVal(val, converted.unit);
                    chartData.options.series[0].data[i].y = Number( parseFloat(val));
                }
            }
            ...
            function changeMultiLineValue(component){
                ...
                _.forEach(chartData.tempData, function(value, seriesCount){
                    for(var i=0; i<value.length; i++){
                        ...
                        chartData.options.series[seriesCount].data[i][1] = Number(parseFloat(val) );
                        if(component.chartType == 'Bubble')
                            chartData.options.series[seriesCount].data[i][2] = Number(parseFloat(val) );
        ...
        //stacked Bar
            ...
            function highchartStackedBarReplace(component, data1){
                if(!data1[0][component.ySeries]){
                    swal('Wrong Series Selected X & Y', 'You selected opoosite series for chart '+ component.title, 'error');
                    return;
                }
                ...
                if( (angular.isDefined(chartData.statement) && chartData.statement.eventPublish == 'Combined') || component.data == 'DBPull' ){
                    ...
                    recurIndex(component, data1, function(keyIndex){
                        for(var seriesCount = 0; seriesCount < data1[keyIndex][component.ySeries].length; seriesCount++){
                            chartData.options.series.push({name: data1[keyIndex][component.ySeries][seriesCount][component.ySeries].toString(), color:highchartProcessData.colorpallete[seriesCount], data: []});
                            chartData.tempData[seriesCount] = [];
                        }
                    })
                }
        ...
        //Pyramid
            ...
            function changePyramidValue(component, converted){
                ...
                for(var i = 0; i < chartData.tempData.length; i++) {
                    var val = chartData.tempData[i];
                    val = getConvertedVal(val, converted.unit);
                    chartData.options.series[0].data[i][1] = Number(parseFloat(val));
                }
            }
        ...
        //Set xAxis label formate
            function xAxisFormate(component){
                if(component.labelType == 'time'){
    ...
    /*Embeded Chart*/
        ...
        function subscribeIboxWithChart(component){
            ...
            if(component.data != 'DBPull'){
                var url = dbService.snapshotUrl({op:'select', id: component.query, limit: component.dataelement});
                httpService.get(url).then(function(res){
                    console.log("componentcomponentSnapshot", $scope.displaydata[component._id].updateTime);
                    if(res.data.length == 0)
                        $scope.displaydata[component._id].loader = false;
                    ...
                    socket.subscribe(component.query, function(res){
                        var ibox = component;
                        console.log("componentcomponent", $scope.displaydata[component._id]);
            ...
            function changeD3ScatterValue(component){
                ...
                _.forEach(component.series, function(value, seriesCount){
                    for(var i = 0; i < chartData.tempData[seriesCount].values.length; i++) {
                        ...
                        val = getConvertedVal(val, converted.unit);
                        chartData.data[seriesCount].values[i].y = parseFloat(val );
                    }
                });
        ...
        //Column Extended Table(Complex Table)
            ...
            function complexTableReplaceData(component, data){
                ...
                if(data.length > 0){
                    ...
                    var dataLength= $scope.currPage*$scope.itemsPerPage;
                    if(dataLength>data.length){
                        dataLength= data.length;
                    }
                    for(var i=($scope.currPage-1)*$scope.itemsPerPage; i<dataLength; i++){
                    ...
                    for(var i=($scope.currPage-1)*$scope.itemsPerPage; i<dataLength; i++){
                        ...
                        for(j in data[i].data){
                            ...
                            processedData[index][component.rowData]= (converted.value).toFixed(2)+converted.unit;
                        }
                        //console.log("processedData",processedData);
                ...
                if( component.gauge.data != 'DBPull' ){
                    ...
                    httpService.get(url).then(function(res){
                        if(res.data.length > 0) {
                            ...
                            processGauge(component.gauge, res.data[0])
                        }
        ...
        function processGauge(component, data){
            ...
            if( component.unit == 'speed' ){
                if( component.unit2 == 'Kbps' ){
                    gaugedata = Number((data[component.kpi] / Math.pow(2, 10)).toFixed(parseInt(component.dataDecimal)) );
                    gaugeunit = 'Kbps';
                }
                else if( component.unit2 == 'Mbps' ){
                    gaugedata = Number((data[component.kpi] / Math.pow(2, 20)).toFixed(parseInt(component.dataDecimal)) );
                    gaugeunit = 'Mbps';
                }
                else if( component.unit2 == 'Gbps' ){
                    gaugedata = Number((data[component.kpi] / Math.pow(2, 30)).toFixed(parseInt(component.dataDecimal)) );
                    gaugeunit = 'Gbps';
                }
            }
                ...
                if( component.unit2 == 'KB' ){
                    ...
                    gaugedata = angular.copy(Number( (data[component.kpi] / Math.pow(2, 30)).toFixed(parseInt(component.dataDecimal)) ));
                    gaugeunit = 'GB';
                }
            }
            else{
                ...
                if(newValue.value){
                    gaugedata = newValue.value.toFixed(component.dataDecimal);
                    gaugeunit = newValue.unit;
                }
            }

            // if(gaugedata > component.max) gaugedata = component.max;
            gauge.data = gaugedata || 0;
    ...
    /*Flot Chart*/
            ...
            function changeFlotPieChart(component, converted){
                ...
                if(converted.unit != ''){
                    for(var i = 0; i < chartData.tempData.length; i++) {
                        var val = chartData.tempData[i];
                        val = getConvertedVal(val, converted.unit);
                        chartData.dataset[i].data = parseFloat(val);
                    }
                }
            }
        ...
        //For Bar chart
            ...
            function changeValueBarFlowChart(component, converted){
                var chartData = $scope.displaydata[component._id];
                chartData.options.yaxis.axisLabel = (component.yAxislabel) ? component.yAxislabel + '('+ converted.unit + ')' : converted.unit;
        ...
        //For Line chart
            ...
            function checkCondition(component, data){
                ...
                console.log("chartData.updateTime", chartData.updateTime);
    ...
    function redirectToOtherPage(params, component){
        ...
        var from = $scope.date.start+'T00:00:00.000Z';
        var to = $scope.date.end+'T23:59:59.999Z';
        params.fromDate= $scope.date.start;
        params.toDate= $scope.date.end;
        var filterParams = '';
            ...
            httpService.get(url).then(function(res){
                ...
                if(angular.isDefined(res.file) && res.type == 'static' ){
                    if(table == 'analysis')
                        ...
                        console.log("paramsComplete",params);
                        $state.go('index.dashboards',{'id': id, 'params': params, 'filterParams': filterParams});
                    }
                    ...
                    if(table == 'analysis')
                        ...
                        if(res.page == 'dashboard')
                            $state.go('index.dashboards',{'name': name, 'file': res.name, 'params': params, 'filterParams': filterParams});
    ...
    function timeInterval(component){
        ...
        if($stateParams.params){
            parameter += '&'+$stateParams.params.Key + '='+ $stateParams.params.label;
        }

        httpService.get(globalConfig.pullfilterdataurl + component.query + parameter).then(function(res){
            if(res.data == 'null' || res.data.length == 0) $scope.displaydata[component._id].loader = false;
            else if( angular.isDefined(res.data) && res.data.length > 0) processReplaceData(component, res);
        });
    }
    ...
    $scope.search = function (date){
        ...
        var granularity = (date.granularity != undefined) ? date.granularity : 0;
        parameter += '&granularity='+ granularity;
        ...
        _.forEach(componentArr, function(value, key){
            ...
            httpService.get(globalConfig.pullfilterdataurl + component.query + parameter + '&queryname='+ component.statement.name).then(function(res){
                if(res.data == 'null' || res.data.length == 0) $scope.displaydata[component._id].loader = false;
                else if( angular.isDefined(res.data) && res.data.length > 0) processReplaceData(component, res);
            });
        });
