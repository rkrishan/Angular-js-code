...
angular.module('specta')
    .controller('StagingViewCtrl', function ($scope, $rootScope, $location, $state, $interval, filterService, $timeout,$sce, $stateParams, $filter, $uibModal, NgMap, globalData, globalConfig, ChartService, SweetAlert, UserProfile, socket, NgTableParams, httpService, dbService, highchartProcessData, currentUser, utility){
    ...
    $scope.dataTblSelectCol = function(id, index, col){
        var table = angular.copy($scope.displaydata[id]);
        var rowData = table.component.rowData
        var rowName = table.component.rowName
        var colName = table.component.colName

        console.log(colName, rowData, rowName, col)
        // console.log(table.data);
        // table.data = [table.data[0]]
        var data = [];
        for(var i in table.data){
            // console.log(table.data[i])
            var label = table.data[i][rowName]
            // console.log('label', table.data[i][rowName], table.data[i].data)
            var tmp = dbService.unique(table.data[i].data, colName, col)[0];
            tmp[rowName] = label
            data.push(tmp)
            //{label: label, value:tmp[colName]}
        }
    ...
    $scope.removeComponent = function (component1){
        ...
        function (isConfirm) {
            if(isConfirm){
                var tempArr = [];
                var updateArr = []
                var mainArr = angular.copy($scope.currentPage.components);
                _.forEach(mainArr, function(item, k){
                    _.forEach(item, function(value, key){
                        if(value.component._id != component.component._id){
                            var tmp = angular.copy(value)
                            tmp.component = {_id: value.component._id, width: value.component.width}
                            updateArr.push(tmp)
                ...
                backgroundUpdatePage({components: updateArr, _id:$scope.currentPage._id, dashboardId: $stateParams.id});
    ...
    function subscribeData(page){
        ...
        angular.forEach(page.components, function(test, key){
            angular.forEach(test, function(value, k){
                var query = '{_id: ObjectId("'+value.component._id+'")}';
                var params = 'query=' + encodeURIComponent(query);
                var url = dbService.makeUrl({collection: 'modules', op:'select', params: params});
                httpService.get(url).success(function (res){
                    if(res.length == 0){
                        swal('', 'Module deleted', 'error')
                        return
                    }
                    res = res[0]
                    res.width = value.component.width
                    value.component = res
                    // $scope.currentPage.components[key] = value;
                    // console.log(key, $scope.currentPage)
                
                    if(value.componentType == 'custom'){
                        $scope.displaydata[value.component._id] = {file : 'views/static/'+ value.component.file};
                    }
    ...
    function subscribeForAllComponent(component1){
        ...
        var type = component1.componentType;
        if(type == 'iBox_Multi_no_Header'){
            ...
            isStatement(component, function(res){
                if(res){
                    ...
                    else if(type == 'multi_gauge')              subscribeGauge(component);
    ...
    /*Map*/
        function subscribeMap(component){
            // component.query = 'aca7fbeca4dc9a9f176f9cb71';
            // component.data = 'CEP';
            ...
            if(component.data != 'DBPull'){
                ...
                //var url = 'http://10.49.28.26:8080/DataAPI/UISnapshot?op=select&id=aca7fbeca4dc9a9f176f9cb71&limit=20';
                httpService.get(url).then(function(res){
                    if(res.data == 'error' && res.data.length == 0)
        ...
        function mapReplaceData(component, data1){
            ...
            if(component.mapType == 'simple'){
                ...
                _.forEach(data1, function(data, key){
                    ...
                    var info = [];
                    //if(lat == -7.344274 && lang == 12.728482) return;
                    ...
                    if(key == data1.length - 1){
                        ...
                        // console.log('map.mapOptions', map.mapOptions, mapcount[0]);
    ...
    /*Highchart*/
        ...
        //Pie Chart
            ...
            function highchartSingleSeriesReplace(component, data1){
                ...
                // console.log(typeof(component.series), component.series, data1)
        ...
        //stacked Bar
            function subscribeStackedBarHighchart( component ){
                ...
                // component.ySeries = 'Latency';
                // component.fixOrder = 1
                if(component.fixOrder){
                    var url = dbService.makeUrl({collection: 'lku_'+component.ySeries.toLowerCase()+'_bkt', op:'select'});
            ...
            function highchartStackedBarReplace(component, data1){
                $scope.displaydata[component._id].origionalData = angular.copy(data1);
                // console.log(component, component.ySeries, data1[0][component.ySeries])
                        ...
                        if(component.fixOrder){
                            for(var z in chartData.fixSeries){
                                ...
                                var isVal = _.filter(data1[keyIndex][component.ySeries], function(item){
                                    return item[component.ySeries] == CEI[component.ySeries]
                                })[0]
                                if(isVal){
                            ...
                            /*for(var z in globalConfig.lookup[component.ySeries]){
                                var CEI = globalConfig.lookup[component.ySeries][z];
                                var isVal = _.filter(data1[keyIndex][component.ySeries], function(item){
                                    return item[component.ySeries] == CEI.value
                                })[0]
                                if(isVal){
                                    chartData.options.series.push({name: isVal[component.ySeries].toString(), color: CEI.color, data: []});
                                    chartData.tempData.push([]);    
                    ...
                    if( keyindex > -1 ){
                        ...
                        for(var seriesCount = 0; seriesCount < chartData.options.series.length; seriesCount++) {
                            ...
                            if(test){
                                ...
                                chartData.options.series[seriesCount].data.push('');
                                chartData.tempData[seriesCount].push('');   
                            }
                        }
                    }
        ...
        //Set xAxis label formate
            function xAxisFormate(component, granularity){
                console.log('xAxisFormate -->',granularity)
                if(component.labelType == 'time'){
    ...
    /*Table*/
        //Column Extended Table(Complex Table)
            function subscribeComplexTable(component){
                ...
                // console.log('component',component)
                if(component.fixOrder){
                    var url = dbService.makeUrl({collection: 'lku_'+component.colName.toLowerCase()+'_bkt', op:'select'});
                    httpService.get(url+'&db=datadb').success(function(res){
                ...
                if(component.fixOrder){
                    ...
                    for(var i=0; i<data.length; i++){
                        for(var j in fixOrder){
                            var tmp = dbService.unique(data[i].data, component.colName, fixOrder[j][component.colName])[0]
                            if(tmp){
            ...
            if( component.data != 'DBPull' ){
                ...
                httpService.get(url).then(function(res){
                    ...
                    socket.subscribe(component.query, function(res){
                        var tmp = JSON.parse(res);
                        var data = tmp[component.query];
                        if(angular.isArray(data.event) && component.plot == 'Combined'){
                            var obj = dbService.unique(data.event, component.plotKey, component.plotValue)[0];
    ...
    /*Gauge*/
            ...
            if( component.data != 'DBPull' ){
                ...
                httpService.get(url).then(function(res){
                    ...
                    socket.subscribe(component.query, function(res){
                        var tmp = JSON.parse(res);
                        var data = tmp[component.query];
                        if(component.type == 'multi_gauge'){
                            multiGaugeProcess(component, data.event);
                        }
                        else{
                            if(angular.isArray(data.event) && component.plot == 'Combined'){
                                var obj = dbService.unique(data.event, component.plotKey, component.plotValue)[0];
                                ...
                                if(angular.isArray(data.event))
                                    processGauge(component, data.event[data.event.length-1]);
                                else
        ...
        function multiGaugeProcess(component, data){
            ...
            httpService.get(url+'&db=datadb').success(function(res){
                // console.log(res);
                var plotData = dbService.unique(res, 'AutoPlot', 1);
                // plotData = [plotData[0]]
                // console.log('plotData', plotData);
                for(var i in plotData){
                    var test = dbService.unique(data, 'OLT', plotData[i].Node)[0];
                    if(test){
                        var tmp = gaugeData(component, test);
                        gauge.options.max = convertToKBMBGB(plotData[i].capacity, tmp.gaugeunit);
                        gauge.multiData.push({
                            options   : gauge.options,
                            value     : Number(tmp.gaugedata),
                            unit      : tmp.gaugeunit,
                            title     : plotData[i].Node,
                            max       : convertToKBMBGB(plotData[i].capacity, tmp.gaugeunit),
                            threshold1: convertToKBMBGB(plotData[i].threshold1, tmp.gaugeunit),
                            threshold2: convertToKBMBGB(plotData[i].threshold2, tmp.gaugeunit)
                        });
                    }
                }
                console.log(gauge);
        ...
        function convertToKBMBGB(value, unit){
            if(unit == 'KB')
                return parseInt( value / Math.pow(2, 10) )
            else if(unit == 'MB')
                return parseInt( value / Math.pow(2, 20) )
            else if(unit == 'GB')
                return parseInt( value / Math.pow(2, 30) )
        }
    /*End Gauge*/
    ...
    function timeInterval(component){
        console.log('timeInterval >>>>', component.title, $scope.date );
        // $scope.date.start = '2017-08-22';
        // $scope.date.end = '2017-08-14';
        ...
        if($stateParams.paramsArray){
            ...
            parameter += '&'+$stateParams.paramsArray[$stateParams.paramsArray.length-1].Key + '='+ $stateParams.paramsArray[$stateParams.paramsArray.length-1].label;
            }
        }
    ...
    function getAdvanceFilterParam(fromValue, toValue,label){
        ...
        if(angular.isDefined(fromValue) && fromValue != ''){
            ...
            if(toValue.length>0){
                ...
                {*/
                    paramAdvanceFilter=paramAdvanceFilter + ",'$lte':"+toValue;
                        return '{'+paramAdvanceFilter+'}';
                /*    }
                else{
                    swal('', 'Enter valid input for usage filter!!', 'error')
                }*/
            }
            else{
                return '{'+paramAdvanceFilter+'}';
    ...
    function processReplaceData(component, data1){
        if(component.plotKey && component.plotKey != '')
